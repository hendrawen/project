...
class Wp_suplier extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'wp_suplier/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'wp_suplier/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'wp_suplier/index.html';
            $config['first_url'] = base_url() . 'wp_suplier/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Wp_suplier_model->total_rows($q);
        $wp_suplier = $this->Wp_suplier_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'wp_suplier_data' => $wp_suplier,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->load->view('wp_suplier/wp_suplier_list', $data);
    }
