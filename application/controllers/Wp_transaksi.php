...
class Wp_transaksi extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Wp_transaksi_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'wp_transaksi/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'wp_transaksi/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'wp_transaksi/index.html';
            $config['first_url'] = base_url() . 'wp_transaksi/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Wp_transaksi_model->total_rows($q);
        $wp_transaksi = $this->Wp_transaksi_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'wp_transaksi_data' => $wp_transaksi,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->load->view('wp_transaksi/wp_transaksi_list', $data);
    }

    public function read($id) 
    {
        $row = $this->Wp_transaksi_model->get_by_id($id);
        if ($row) {
            $data = array(
		'id' => $row->id,
		...
		'wp_status_id' => $row->wp_status_id,
            ...
            $this->load->view('wp_transaksi/wp_transaksi_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('wp_transaksi'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('wp_transaksi/create_action'),
	    'id' => set_value('id'),
        ...
        $this->load->view('wp_transaksi/wp_transaksi_form', $data);
    }
    ...
    public function create_action() 
    {
		...
		'wp_status_id' => $this->input->post('wp_status_id',TRUE),
            ...
            $this->Wp_transaksi_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('wp_transaksi/update_action'),
    ...
    public function update_action() 
    {
		...
		'wp_status_id' => $this->input->post('wp_status_id',TRUE),
            ...
            $this->Wp_transaksi_model->update($this->input->post('id', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Wp_transaksi_model->delete($id);
	...
	foreach ($this->Wp_transaksi_model->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        header("Content-Disposition: attachment;Filename=wp_transaksi.doc");

        $data = array(
            'wp_transaksi_data' => $this->Wp_transaksi_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('wp_transaksi/wp_transaksi_doc',$data);
    }

}

/* End of file Wp_transaksi.php */
/* Location: ./application/controllers/Wp_transaksi.php */
