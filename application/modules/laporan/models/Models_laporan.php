...
class Models_laporan extends CI_Model {
    ...
    {
        $query = $this->db->query("SELECT wp_transaksi.id_transaksi, wp_transaksi.tgl_transaksi, DATE_ADD(wp_transaksi.tgl_transaksi, INTERVAL 14 DAY) as jatuh_tempo, wp_pelanggan.id_pelanggan, wp_pelanggan.nama_pelanggan, wp_barang.nama_barang, SUM(wp_transaksi.qty) as qty, wp_barang.satuan, wp_pelanggan.kelurahan, wp_pelanggan.kecamatan,wp_pelanggan.no_telp, wp_karyawan.nama, wp_transaksi.username, SUM(wp_asis_debt.turun_krat) AS total, wp_penarikan.tgl_penarikan, wp_asis_debt.bayar_krat, wp_penarikan.tgl_penarikan, wp_asis_debt.bayar_uang, sum(wp_asis_debt.bayar_krat+(wp_asis_debt.bayar_uang/wp_krat_kosong.harga)) as jumlah, (sum(wp_asis_debt.turun_krat) - sum(wp_asis_debt.bayar_krat+(wp_asis_debt.bayar_uang/wp_krat_kosong.harga))) as sisa, IF (sum(wp_asis_debt.turun_krat) > (sum(wp_asis_debt.bayar_krat+(wp_asis_debt.bayar_uang/wp_krat_kosong.harga))), 'Masih ada ASET', 'Tidak ada ASET') as status
        FROM
        wp_asis_debt
        LEFT JOIN
        wp_penarikan
    ...
    function laporan_debtahunan($year, $nama)
    {
        ...
        if ($nama !== 'semua') {
          ...
          if ($day !== 'semua') {
            $this->db->where('wp_transaksi.tgl_transaksi', $year);
          }
        $this->db->where('wp_pembayaran.bayar >=', 0);
        $this->db->join('wp_transaksi', 'wp_pembayaran.id_transaksi = wp_transaksi.id_transaksi');
        $this->db->join('wp_pelanggan', 'wp_transaksi.wp_pelanggan_id = wp_pelanggan.id');
