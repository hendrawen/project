...
class Penjualan2 extends CI_Controller {
  ...
  public function __construct()
  {
      ...
      $this->load->model('Penjualan_model', 'mLap');
  ...
  function ajax_list()
  {
    ...
    $list = $this->mLap->get_datatables();
      ...
      foreach ($list as $lap) {
          ...
          $row[] = $lap->nama_debt;
          $row[] = $lap->nama_status;
          $row[] = number_format($lap->subtotal);
          $data[] = $row;
      }
  ...
  function excel_tanggal($tanggal, $status)
  {
    ...
    xlsWriteLabel(1, 1, $tanggal);
    xlsWriteLabel(2, 0, "Status");
    xlsWriteLabel(2, 1, $this->mLap->get_status_id($status));
    $kolomhead = 0;
    ...
    $record = $this->mLap->laporan_tanggal($tanggal, $status);
    $total = 0;
    if ($record){
  ...
  function excel_bulan($dari, $ke, $tahun, $status)
  {
    $this->load->helper('exportexcel');
    $namaFile = "transaksi_bulan.xls";
    $judul = "Transaksi";
    $tablehead = 6;
    $tablebody = 7;
    $nourut = 1;
    ...
    xlsWriteLabel(1, 1, $dari);

    xlsWriteLabel(2, 0, "Ke");
    xlsWriteLabel(2, 1, $ke);

    xlsWriteLabel(3, 0, "Tahun");
    xlsWriteLabel(3, 1, $tahun);

    xlsWriteLabel(4, 0, "Status");
    xlsWriteLabel(4, 1, $this->mLap->get_status_id($status));
    ...
    $record = $this->mLap->laporan_bulan($dari, $ke, $tahun, $status);
    $total = 0;
    if ($record){
      foreach ($record as $data) {
          $kolombody = 0;
  ...
  function excel_tahun($tahun, $status)
  {
    $this->load->helper('exportexcel');
    $namaFile = "transaksi_tahun.xls";
    $judul = "Transaksi";
    $tablehead = 4;
    $tablebody = 5;
    $nourut = 1;
    ...
    $record = $this->mLap->laporan_tahun($tahun, $status);
    $total = 0;
    if ($record){
      foreach ($record as $data) {
          $kolombody = 0;
