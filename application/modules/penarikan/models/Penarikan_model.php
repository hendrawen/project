...
class Penarikan_model extends CI_Model{
  ...
  var $order = array('wp_transaksi.id_transaksi' => 'DESC'); // default order
    ...
    public function get_by_id($no_faktur)
    {
        ...
        // wp_pelanggan.id_pelanggan, wp_pelanggan.nama_pelanggan, wp_asis_debt.tanggal as tgl_penarikan, wp_asis_debt.bayar_krat, wp_asis_debt.bayar_uang, wp_penarikan.id_penarikan');
         ...
         * Baru
         */
        $this->db->select('wp_penarikan.*, wp_pelanggan.id_pelanggan, wp_pelanggan.nama_pelanggan, wp_asis_debt_id as id_asis_debt');
        $this->db->join('wp_asis_debt', 'wp_asis_debt.id = wp_penarikan.wp_asis_debt_id', 'inner');
        $this->db->join('wp_transaksi', 'wp_transaksi.id = wp_asis_debt.id_transaksi', 'inner');
        $this->db->join('wp_pelanggan', 'wp_pelanggan.id = wp_penarikan.wp_pelanggan_id', 'inner');
        $this->db->where('wp_transaksi.id_transaksi', $no_faktur);
        return $this->db->get('wp_penarikan')->result();
    }
    ...
    function get_penarikan($id)
    {
        $this->db->select('wp_penarikan.*, wp_asis_debt.id as id_asis_debt');
        
        $this->db->where('id_penarikan', $id);
        $this->db->join('wp_asis_debt', 'wp_asis_debt.id = wp_penarikan.wp_asis_debt_id', 'inner');
        return $this->db->get('wp_penarikan')->row();
    }

    public function update_penarikan($penarikan, $asis_debt)
    {
        //   updating wp_penarikan
        $id_penarikan = $penarikan['id_penarikan'];
        unset($penarikan['id_penarikan']);
        $this->db->where('id_penarikan', $id_penarikan);
        $this->db->update('wp_penarikan', $penarikan);
        // updating wp_asis_debt
        $id = $asis_debt['id'];
        unset($asis_debt['id']);
        $this->db->where('id', $id);
        $this->db->update('wp_asis_debt', $asis_debt);

    }

    public function get_asis_debt($id)
    {
        $this->db->select('id, bayar_krat, bayar_uang, piutang');
        $this->db->where('id', $id);
        return $this->db->get('wp_asis_debt')->row();
    }

    public function harga_krat_kosong()
    {
        $this->db->select('harga');
        $this->db->where('id', 1);
        $hasil = $this->db->get('wp_krat_kosong', 0, 1)->row();
        return $hasil->harga;
    }
}
