$(document).ready(function(){
  $('.add_cart').click(function(){
    ...
    //console.log(idpesan);
    $.ajax({
      url : (base_url+"hrd/pembelian/add_to_cart"),
      method : "POST",
...
$('#detail_cart').load(base_url+"hrd/pembelian/load_cart");
...
$('#id_barang').on('input',function(){
           ...
           $.ajax({
               type : "POST",
               url  : (base_url+"hrd/pembelian/get_barang"),
               dataType : "JSON",
  ...
  $('#wp_suplier_id').autocomplete({
          source: (base_url+"hrd/pembelian/get_autocomplete"),
  ...
  $('#id_suplier').on('input',function(){
             ...
             $.ajax({
                 type : "POST",
                 url  : (base_url+"hrd/pembelian/get_suplier"),
                 dataType : "JSON",
  ...
  $(document).on('click','.romove_cart',function(){
    ...
    $.ajax({
      url : (base_url+"hrd/pembelian/delete_cart"),
      method : "POST",
      data : {row_id : row_id},
      success :function(data){
        $('#detail_cart').html(data);
  ...
  $(document).on('click','.hapus_cart',function(){
    ...
    $.ajax({
      url : (base_url+"hrd/pembelian/hapus_cart"),
      method : "POST",
      success :function(data){
        $('#detail_cart').html(data);
      }
     ...
     console.log(judul);
      if(judul!=""){
          $("#result2").html(base_url+"assets/ajax-loader.gif");
            $.ajax({
                   type : "POST",
                url  : (base_url+"hrd/pembayaran_barang/track_pembayaran"),
                data:"judul="+judul,
