...
$('#title').autocomplete({
        source: (base_url+"bendahara/pembayaran_barang/get_autocomplete"),
...
$('#id_pembayaran_aset').autocomplete({
    source: (base_url+"bendahara/pembayaran_aset/get_auto"),
    select: function (event, ui) {
        $('[name="title"]').val(ui.item.label);
    }
});
       ...
       console.log(judul);
        if(judul!=""){
            $("#result2").html(base_url+"assets/ajax-loader.gif");
              $.ajax({
                     type : "POST",
                  url  : (base_url+"bendahara/pembayaran_barang/track_transaksi"),
                  data:"judul="+judul,
