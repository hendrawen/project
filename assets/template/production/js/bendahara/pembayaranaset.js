...
$("#button_aset").click(function(){
    ...
    $.ajax({
        type: "POST",
        url: base_url+"bendahara/pembayaran_aset/cek_data",
        data: {sp : sp},
        dataType: "html",
...
$("#btn-bayar-aset").click(function () {
    ...
    if (conf == false) {
        ...
        $.ajax({
            type: "POST",
            url: base_url+"bendahara/pembayaran_aset/bayar_aset",
            data: {
                tgl : tgl,
                jenis : jenis,
            ...
            success: function (response) {
                if (response.status == true) {
                    ...
                    $("#tabel_cari_aset").hide();
                    $("#id_track_aset").val('');
                    document.location = base_url+'bendahara/pembayaran_aset/penarikan';
                } else {
                    status = 'F';
...
function get_id_sup(id) {
    $.ajax({
        url: base_url+"bendahara/pembayaran_aset/get_id_sup/"+id,
        dataType: "json",
        success: function (response) {
            $("#id").val(response);
        }
...
function get_total_harga(krat) {
    $.ajax({
        url: base_url+"bendahara/pembayaran_aset/get_harga_krat",
        dataType: "json",
        success: function (response) {
