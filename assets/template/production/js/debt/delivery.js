...
$("#button_aset").click(function(){
    ...
    $.ajax({
        type: "POST",
        url: base_url+"debt/penarikan/cek_data",
...
$("#btn-bayar-aset").click(function () { 
    ...
    if (conf == false) {
        ...
        $.ajax({
            type: "POST",
            url: base_url+"debt/penarikan/bayar_aset",
            data: {
                tgl : tgl, 
                jenis : jenis,
            ...
            success: function (response) {
                if (response.status == true) {
                    ...
                    $("#tabel_cari_aset").hide();
                    $("#id_track_aset").val('');
                    document.location = base_url+'debt/penarikan';
                } else {
                    status = 'F';
...
function get_id_pelanggan(id) {
    $.ajax({
        url: base_url+"debt/penarikan/get_idpelanggan/"+id,
        dataType: "json",
        success: function (response) {
            $("#id").val(response);
        }
