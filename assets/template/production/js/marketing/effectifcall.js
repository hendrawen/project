...
$(document).ready(function () {
    table = $('#table_marketing').dataTable({
        ...
        "ajax": {
            "url": (base_url + "marketing/takeorder/ajax_list"),
            "type": "POST",
            "data": function (data) {
...
$('#btn-filter12').click(function () { //button filter event click
...
function reload_table()
      {
           $('#table_marketing').DataTable().ajax.reload();//reload datatable ajax
      }
...
function delete_call(id) {
    if (confirm('Are you sure delete this data?')) {
        ...
        $.ajax({
            url: (base_url + "marketing/takeorder//ajax_delete/" + id),
            type: "POST",
            dataType: "JSON",
